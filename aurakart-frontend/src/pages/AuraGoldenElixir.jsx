...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiDroplet } from 'react-icons/fi';
...
const AuraGoldenElixir = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Golden Elixir added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      addOrder({
        total: (110.00) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
  ...
  const specs = [
    { label: "Infusion", value: "24K Pure Gold Flakes", icon: <FiStar className="text-orange-400" /> },
    { label: "Base", value: "Organic Cold-Pressed Rosehip", icon: <FiDroplet className="text-blue-400" /> },
    { label: "Hydrate", value: "Pure Plant Squalane", icon: <FiZap className="text-purple-400" /> },
    { label: "Finish", value: "Non-Greasy Velvet Glow", icon: <FiCheckCircle className="text-green-400" /> },
    { label: "Purity", value: "Chemical-Free Artisan Grade", icon: <FiShield className="text-cyan-400" /> },
    { label: "Warranty", value: "Authenticity Certified", icon: <FiCheckCircle className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Luxe Collection Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Oil" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-amber-500 to-rose-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src={product.image} alt="Golden Elixir" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-amber-500/20 text-amber-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-amber-500/30">Ultimate Luxury Radiance</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Golden <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-rose-400">Elixir Face Oil</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The pinnacle of botanical luxury. Infused with 24K gold flakes and pure rosehip oil, the Aura Golden Elixir provides deep nourishment and an instant, velvet-smooth glow for all skin types.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-amber-500/20 text-amber-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Pure Gold Edition</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-amber-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Ethical Source Cert</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Luxe Delivery</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-rose-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-rose-500/10 rounded-full border border-rose-500/20">Artisan Formula</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Pure Performance</h2><div className="w-32 h-1.5 bg-gradient-to-r from-rose-500 via-amber-400 to-rose-500 rounded-full shadow-[0_0_20px_rgba(244,63,94,0.4)]"></div></div>
...
export default AuraGoldenElixir;

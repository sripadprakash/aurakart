...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiCpu, FiSmartphone, FiBattery, FiCamera, FiWifi, FiActivity, FiHardDrive, FiMapPin, FiMonitor, FiCpu as FiProcessor } from 'react-icons/fi';
...
const AuraBookPro16 = () => {
  ...
  const product = {
    id: 'elec-laptop-1',
    name: "AuraBook Pro 16 (M3 Ultra, 32GB RAM, 1TB SSD)",
    price: 2499.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('AuraBook Pro added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Your professional workstation is on its way.', {
        icon: '💻',
        duration: 5000,
  ...
  const specs = [
    { label: "Processor", value: "Aura M3 Ultra Chip", icon: <FiProcessor className="text-orange-400" /> },
    { label: "Memory", value: "32GB Unified RAM", icon: <FiActivity className="text-blue-400" /> },
    { label: "Display", value: "16\" Liquid Retina XDR", icon: <FiMonitor className="text-purple-400" /> },
    { label: "Battery", value: "22-Hour Runtime", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Storage", value: "1TB Super-Fast SSD", icon: <FiHardDrive className="text-cyan-400" /> },
    { label: "Warranty", value: "3 Year ProCare+", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Ultimate Workstation Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Laptop" />
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          <div className="flex justify-between items-center mb-8">
            <Link to="/category/electronics" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors group">
              <FiArrowLeft className="group-hover:-translate-x-1 transition-transform" />
              <span>Back to Electronics</span>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Laptop" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-video" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-blue-500/30">Ultimate Professional Workstation</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">AuraBook <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Pro 16 (M3 Ultra)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The most powerful laptop ever built. Featuring the M3 Ultra chip, 32GB unified memory, and a stunning 16" Liquid Retina XDR display for uncompromising creators.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Pro Series</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">3 Year Pro Warranty</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Next-Day Priority Delivery</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-blue-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">Creator Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-blue-500 via-indigo-400 to-blue-500 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.4)]"></div></div>

...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiCpu, FiSmartphone, FiBattery, FiCamera, FiWifi, FiActivity, FiHardDrive, FiMapPin } from 'react-icons/fi';
...
const AuraPhoneProMax = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('AuraPhone Pro Max added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (899) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Your new AuraPhone is on the way.', {
        icon: '📱',
        duration: 5000,
  ...
  const specs = [
    { label: "Processor", value: "Aura A18 Bionic Elite", icon: <FiCpu className="text-orange-400" /> },
    { label: "Memory", value: "12 GB RAM, 256 GB", icon: <FiHardDrive className="text-blue-400" /> },
    { label: "Camera", value: "200MP Quad Pro System", icon: <FiCamera className="text-purple-400" /> },
    { label: "Battery", value: "5200 mAh", icon: <FaBatteryFull className="text-green-400" /> },
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Red Edition (12GB/256GB)</p></div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Pro Max" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Red 12 GB RAM, 256 GB Edition</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">AuraPhone <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-400">Pro Max 5G (Red, 12GB RAM, 256GB)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Experience the peak of mobile engineering. The AuraPhone Pro Max 5G (Red, 12GB RAM, 256GB) delivers unmatched performance with the A18 Bionic Elite chip and a revolutionary 200MP camera system.</p>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">3 Year AuraCare+</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Global Express Delivery</span></div></div>
            </motion.div>
          </div>
...
export default AuraPhoneProMax;

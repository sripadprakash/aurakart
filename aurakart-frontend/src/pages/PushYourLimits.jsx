// src/pages/PushYourLimits.jsx
import { useState, useEffect } from 'react';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import { motion, AnimatePresence } from 'framer-motion';
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiCreditCard, FiActivity, FiTarget, FiWind, FiDroplet, FiMapPin } from 'react-icons/fi';
...
const PushYourLimits = () => {
  ...
  // --- WISHLIST SYNC ---
  const isWishlisted = user?.wishlist?.some(item => item.id === 'activewear-1');
  ...
  const handleWishlistClick = (e) => {
    ...
    if (isWishlisted) {
      updatedWishlist = currentWishlist.filter(item => item.id !== 'activewear-1');
      toast.success('Removed from wishlist');
    } else {
      const newItem = {
        id: 'activewear-1',
        name: 'AuraElite Seamless Activewear',
        price: 59.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Added to basket! Buy 2 Get 1 Free active.', { icon: '🔥' });
  };
  ...
  const handleBuyNow = (e) => {
    ...
    setShowCheckout(true);
    toast.success('B2G1 Active: Buy 2 Get 1 Free Applied!', { icon: '🔥', duration: 3000 });
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (119.98) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 3, // B2G1
        productName: 'AuraElite Activewear Bundle'
      });

      toast.success('Purchase Successful! Get ready to train.', {
        icon: '🏃',
        duration: 5000,
  ...
  const specs = [
    { label: "Fabric", value: "Aura-Dry Tech", icon: <FiZap className="text-yellow-400" /> },
    { label: "Elasticity", value: "4-Way Elite Flex", icon: <FiTarget className="text-blue-400" /> },
    { label: "Ventilation", value: "Smart Air-Flow Mesh", icon: <FiWind className="text-cyan-400" /> },
    { label: "Moisture", value: "WR-Hydra Repel", icon: <FiDroplet className="text-blue-500" /> },
    { label: "Construction", value: "Zero-Chafe Seams", icon: <FiShield className="text-green-400" /> },
    { label: "Compression", value: "High-Core Support", icon: <FiActivity className="text-red-400" /> },
  ];
  ...
  return (
    <div className="min-h-screen text-white font-sans flex flex-col relative overflow-hidden">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Performance Gear Bundle</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src="https://images.unsplash.com/photo-1518310383802-640c2de311b2?q=80&w=200&auto=format&fit=crop" className="w-16 h-16 rounded-xl object-cover" alt="Activewear" />
                  <div className="flex-1">
                    <h4 className="font-bold text-sm">AuraElite Activewear</h4>
                  ...
                  <div className="text-right"><p className="font-black text-lg">$119.98</p><p className="text-[10px] text-green-500 font-bold uppercase">+ 1 FREE ITEM</p></div>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm"><span className="text-gray-400 font-medium">Subtotal (3 items)</span><span className="font-bold">$179.97</span></div>
                  <div className="flex justify-between text-sm text-green-400"><span className="font-medium text-gray-400">B2G1 Discount</span><span className="font-black">-$59.99</span></div>
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-center"><span className="text-lg font-bold">Total</span><span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">${((119.98) + (user?.orders?.length >= 3 ? 1 : 0)).toFixed(2)}</span></div>
                </div>
      ...
      <div className="fixed inset-0 z-0 bg-[#020617] overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[45rem] h-[45rem] bg-red-600/10 rounded-full blur-[150px]"></div>
      </div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-orange-500 to-red-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src="https://images.unsplash.com/photo-1518310383802-640c2de311b2?q=80&w=2000&auto=format&fit=crop" alt="Activewear" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div><span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Activewear Exclusive</span><h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Push Your <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-400">Limits</span></h1><div className="flex items-center gap-4 mb-4 text-green-400 font-black text-xs tracking-widest uppercase animate-pulse bg-green-500/10 p-2 rounded-lg border border-green-500/20">Special: Buy 2 Get 1 Free Applied</div></div>
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Engineered for peak performance. Aura-Dry technology meets 4-way elite flex for ultimate freedom of movement and maximum comfort during your most intense sessions.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">$59.99</span><span className="text-gray-400 line-through text-xl">$89.99</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Elite Durability</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Free Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-orange-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-orange-500/10 rounded-full border border-orange-500/20">Athlete Standard</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-orange-500 via-red-400 to-orange-500 rounded-full shadow-[0_0_20px_rgba(249,115,22,0.4)]"></div></div>

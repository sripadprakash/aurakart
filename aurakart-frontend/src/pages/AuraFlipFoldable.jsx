...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiCpu, FiSmartphone, FiBattery, FiCamera, FiWifi, FiActivity, FiHardDrive, FiMapPin, FiMaximize } from 'react-icons/fi';
...
const AuraFlipFoldable = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Aura Flip Foldable added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (999) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! The Future is in your hands.', {
        icon: '✨',
        duration: 5000,
  ...
  const specs = [
    { label: "Processor", value: "Aura Dimensity 9000+", icon: <FiCpu className="text-orange-400" /> },
    { label: "Memory", value: "12GB RAM, 256GB", icon: <FiHardDrive className="text-blue-400" /> },
    { label: "Display", value: "7.2\" Foldable AMOLED", icon: <FiMaximize className="text-purple-400" /> },
    { label: "Battery", value: "4800 mAh", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Network", value: "5G Ultra Connect", icon: <FiWifi className="text-cyan-400" /> },
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Flip Foldable Edition (12GB/256GB)</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Phone" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Flip Foldable" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-purple-500/20 text-purple-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-purple-500/30">12GB RAM, 256GB Edition</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Flip Foldable (12GB RAM, 256GB)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The future is here with a compact foldable design and a dynamic outer display. The Aura Flip Foldable (12GB RAM, 256GB) delivers cutting-edge technology in a pocket-sized package.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-purple-500/20 text-purple-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">New Tech</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-purple-400 text-2xl" /><span className="text-sm font-bold text-gray-200">3 Year AuraCare+</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-pink-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Priority Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-purple-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-purple-500/10 rounded-full border border-purple-500/20">Masterpiece Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-purple-500 via-pink-400 to-purple-500 rounded-full shadow-[0_0_20px_rgba(168,85,247,0.4)]"></div></div>
...
export default AuraFlipFoldable;

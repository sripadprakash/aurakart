...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiSettings, FiTarget, FiFeather } from 'react-icons/fi';
...
const AuraHeritageAutomatic = () => {
  ...
  const product = {
    id: 'watch-classic-1',
    name: "Aura Heritage Automatic (24-Jewel, Open Heart)",
    price: 899.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Heritage Automatic added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Horological Excellence Secured.', {
        icon: '⌚',
        duration: 5000,
  ...
  const specs = [
    { label: "Movement", value: "24-Jewel Aura Automatic", icon: <FiSettings className="text-orange-400" /> },
    { label: "Glass", value: "Sapphire Crystal AR-Coated", icon: <FiZap className="text-blue-400" /> },
    { label: "Dial", value: "Open Heart Artisan Design", icon: <FiTarget className="text-purple-400" /> },
    { label: "Power", value: "42-Hour Reserve Kinetic", icon: <FiActivity className="text-green-400" /> },
    { label: "Strap", value: "Premium Italian Calfskin", icon: <FiFeather className="text-cyan-400" /> },
    { label: "Warranty", value: "5 Year Prestige Support", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Heritage Grade Masterpiece</p></div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Heritage Automatic" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Horological Masterpiece</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Heritage <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-400">Automatic (24-Jewel)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Timeless precision for the modern era. The Aura Heritage features a meticulously calibrated 24-jewel automatic movement and a stunning open-heart dial that reveals the mechanical soul within.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-orange-500/20 text-orange-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Heritage Grade</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">5 Year Prestige Support</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Insured Priority Delivery</span></div></div>
            </motion.div>
          </div>
...
export default AuraHeritageAutomatic;

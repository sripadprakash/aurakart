...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiCpu, FiVideo, FiDribbble, FiWifi, FiSmartphone } from 'react-icons/fi';
...
const AuraPeakSmartGym = () => {
  ...
  const handleAddToCart = () => {
    addToCart(product);
    setCartTooltip('Add to your');
    setIsAdded(true);
    toast.success('Smart Gym added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Your Home Evolution Begins.', {
        icon: '💪',
        duration: 5000,
  ...
  const specs = [
    { label: "Resistance", value: "AI-Digital (Up to 200lbs)", icon: <FiCpu className="text-cyan-400" /> },
    { label: "Display", value: "32\" 4K Touch Interface", icon: <FiSmartphone className="text-blue-400" /> },
    { label: "Tracking", value: "3D Form-Correction Vision", icon: <FiVideo className="text-purple-400" /> },
    { label: "Connectivity", value: "Ultra-Low Latency Wi-Fi 6", icon: <FiWifi className="text-green-400" /> },
    { label: "Programs", value: "5000+ On-Demand Classes", icon: <FiDribbble className="text-emerald-400" /> },
    { label: "Warranty", value: "5 Year Hardware Care", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Smart Gym Ecosystem</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Gym" />
                ...
                <button onClick={handleCompletePurchase} disabled={isProcessing} className={`w-full py-5 rounded-2xl font-black text-lg transition-all duration-300 flex items-center justify-center gap-3 ${isProcessing ? 'bg-white/10 text-white/30' : 'bg-white text-black hover:bg-cyan-500 hover:text-white shadow-xl shadow-cyan-500/20'}`}>
                  {isProcessing ? (<><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full" /><span>INITIALIZING...</span></>) : (<><FiZap className="text-xl" />{location === 'Select Location' ? 'SELECT ADDRESS' : 'COMPLETE PURCHASE'}</>)}
                </button>
              </div>
            </motion.div>
          </div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src={product.image} alt="Smart Gym" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-cyan-500/20 text-cyan-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-cyan-500/30">Future of Fitness</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Peak <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400">Smart Gym Elite</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The complete studio in your home. Powered by AI resistance technology and real-time form correction, Aura Peak delivers a personalized, world-class training experience on a stunning 32" touchscreen.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Smart Choice</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-cyan-400 text-2xl" /><span className="text-sm font-bold text-gray-200">5 Year Hardware Care</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">White-Glove Tech Install</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-cyan-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-cyan-500/10 rounded-full border border-cyan-500/20">Evolution Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-cyan-500 via-blue-400 to-cyan-500 rounded-full shadow-[0_0_20px_rgba(6,182,212,0.4)]"></div></div>
...
export default AuraPeakSmartGym;

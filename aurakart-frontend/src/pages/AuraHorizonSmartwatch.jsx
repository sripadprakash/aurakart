...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiCpu, FiSmartphone, FiBattery, FiActivity as FiHealth } from 'react-icons/fi';
...
const AuraHorizonSmartwatch = () => {
  ...
  const product = {
    id: 'watch-smart-1',
    name: "Aura Horizon Smartwatch (Titanium, Sapphire)",
    price: 449.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Horizon Smartwatch added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      addOrder({
        total: (449.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Smart Companion Shipped.', {
        icon: '⌚',
        duration: 5000,
  ...
  const specs = [
    { label: "Case", value: "Grade 5 Aerospace Titanium", icon: <FiShield className="text-orange-400" /> },
    { label: "Display", value: "Always-On Retina OLED", icon: <FiSmartphone className="text-blue-400" /> },
    { label: "Health", value: "Bio-Metric Aura Scan", icon: <FiHealth className="text-purple-400" /> },
    { label: "Battery", value: "14-Day Ultra Performance", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Glass", value: "Military-Grade Sapphire", icon: <FiZap className="text-cyan-400" /> },
    { label: "Warranty", value: "2 Year Global ProCare", icon: <FiCheckCircle className="text-red-400" /> },
  ];
  ...
  return (
    <div className="min-h-screen text-white font-sans flex flex-col relative overflow-hidden">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Smart Horizon Edition</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Smartwatch" />
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-center"><span className="text-lg font-bold">Total</span><span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">${((product.price) + (user?.orders?.length >= 3 ? 1 : 0)).toFixed(2)}</span></div>
                </div>
                <button onClick={handleCompletePurchase} disabled={isProcessing} className={`w-full py-5 rounded-2xl font-black text-lg transition-all duration-300 flex items-center justify-center gap-3 ${isProcessing ? 'bg-white/10 text-white/30' : 'bg-white text-black hover:bg-blue-500 hover:text-white shadow-xl shadow-blue-500/20'}`}>
                  {isProcessing ? (<><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full" /><span>SYNCING...</span></>) : (<><FiZap className="text-xl" />{location === 'Select Location' ? 'SELECT ADDRESS' : 'COMPLETE PURCHASE'}</>)}
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <div className="fixed inset-0 z-0 bg-[#020617] overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[45rem] h-[45rem] bg-indigo-600/10 rounded-full blur-[150px]"></div>
      </div>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          <div className="flex justify-between items-center mb-8">
            <Link to="/category/watches" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors group">
              <FiArrowLeft className="group-hover:-translate-x-1 transition-transform" />
              <span>Back to Horology</span>
            </Link>
            {user && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src={product.image} alt="Smartwatch" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-blue-500/30">High-Tech Wearable</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Horizon <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Smartwatch</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Intelligence on your wrist. The Aura Horizon merges flagship-level craftsmanship with advanced biometric technology and a 14-day battery life that keeps you connected through any journey.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Smart Grade</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">2 Year Global ProCare</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Smart Delivery</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-blue-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">Intelligence Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-blue-500 via-indigo-400 to-blue-500 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.4)]"></div></div>
...
export default AuraHorizonSmartwatch;

// src/pages/SmartTechWrist.jsx
import { useState, useEffect } from 'react';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import { motion, AnimatePresence } from 'framer-motion';
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiWatch, FiBattery, FiBluetooth, FiZap, FiActivity, FiX, FiCheckCircle, FiCreditCard, FiWind, FiDroplet, FiMapPin } from 'react-icons/fi';
...
const SmartTechWrist = () => {
  ...
  // --- WISHLIST SYNC ---
  const isWishlisted = user?.wishlist?.some(item => item.id === 'smart-watch-1');
  ...
  const handleWishlistClick = (e) => {
    ...
    if (isWishlisted) {
      updatedWishlist = currentWishlist.filter(item => item.id !== 'smart-watch-1');
      toast.success('Removed from wishlist');
    } else {
      const newItem = {
        id: 'smart-watch-1',
        name: 'Aura Series 9 Premium Smartwatch',
        price: 349.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Added to cart! $50.00 discount applied.');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (349.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: 'Aura Series 9'
      });

      toast.success('Purchase Successful! Secure delivery initiated.', {
        icon: '⌚',
        duration: 5000,
  ...
  const specs = [
    { label: "Display", value: "Retina Always-On OLED", icon: <FiWatch className="text-blue-400" /> },
    { label: "Battery Life", value: "Up to 36 Hours", icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-green-400 w-8 h-8">
    ...
    { label: "Sensors", value: "Blood Oxygen & ECG", icon: <FiActivity className="text-red-400" /> },
    { label: "Durability", value: "IP6X Dust Resistant", icon: <FiWind className="text-yellow-400" /> },
    { label: "Water Resistance", value: "WR50 Swimproof", icon: <FiDroplet className="text-cyan-400" /> },
    { label: "Connectivity", value: "GPS + Cellular", icon: <FiBluetooth className="text-purple-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Aura Series 9 Edition</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src="https://images.unsplash.com/photo-1523275335684-37898b6baf30?q=80&w=200&auto=format&fit=crop" className="w-16 h-16 rounded-xl object-cover" alt="Smartwatch" />
                  <div className="flex-1">
                    <h4 className="font-bold text-sm">Aura Series 9</h4>
                  ...
                  <div className="text-right">
                    <p className="font-black text-lg">$349.99</p>
                    <p className="text-[10px] text-green-500 font-bold uppercase">-$50.00 OFF</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm"><span className="text-gray-400 font-medium">Subtotal</span><span className="font-bold">$399.99</span></div>
                  <div className="flex justify-between text-sm text-green-400"><span className="font-medium text-gray-400">Flat Discount</span><span className="font-black">-$50.00</span></div>
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-center"><span className="text-lg font-bold">Total</span><span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">${((349.99) + (user?.orders?.length >= 3 ? 1 : 0)).toFixed(2)}</span></div>
                </div>
                <button onClick={handleCompletePurchase} disabled={isProcessing} className={`w-full py-5 rounded-2xl font-black text-lg transition-all duration-300 flex items-center justify-center gap-3 ${isProcessing ? 'bg-white/10 text-white/30' : 'bg-white text-black hover:bg-blue-500 hover:text-white shadow-xl'}`}>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src="https://images.unsplash.com/photo-1523275335684-37898b6baf30?q=80&w=1999&auto=format&fit=crop" alt="Smart Tech" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-blue-500/30">Trending Tech</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Smart Tech <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">on Your Wrist</span></h1>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="text-gray-400 font-medium">(2.4k Reviews)</span>
                </div>
              </div>
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Meet Aura Series 9. The ultimate tool for a healthy life is now even more powerful. Advanced health sensors, faster charging, and a brighter display.</p>
              
              <div className="flex items-center gap-6 my-4">
                <span className="text-5xl font-black text-white">$349.99</span>
                <div className="flex flex-col items-start">
                  <span className="text-xl text-gray-500 line-through font-bold">$399.99</span>
                  <span className="text-blue-400 text-sm font-black uppercase tracking-wider text-left">Flat $50.00 OFF Applied</span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-4">
                ...
                <div className="flex-1">
                  <button onClick={handleAddToCart} className={`cart-btn !bg-[#f97316] !h-16 !rounded-2xl ${isAdded ? 'is-added' : ''}`} data-tooltip="Add to your">
                    <div className="cart-btn-wrapper">
                      <div className="text uppercase tracking-[0.1em] text-lg font-black flex items-center justify-center">ADD TO CART</div>
                      <span className="icon flex-col gap-1">
                        <svg viewBox="0 0 16 16" fill="currentColor" height="24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M0 2.5A.5.5 0 0 1 .5 2H2a.5.5 0 0 1 .485.379L2.89 4H14.5a.5.5 0 0 1 .485.621l-1.5 6A.5.5 0 0 1 13 11H4a.5.5 0 0 1-.485-.379L1.61 3H.5a.5.5 0 0 1-.5-.5zM3.14 5l1.25 5h8.22l1.25-5H3.14zM5 13a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm-2 1a2 2 0 1 1 4 0 2 2 0 0 1-4 0zm9-1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm-2 1a2 2 0 1 1 4 0 2 2 0 0 1-4 0z"></path></svg>
                        <span className="text-[10px] uppercase font-bold tracking-widest leading-none">BASKET</span>
                      </span>
                    </div>
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8">
                <div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
                  ...
                  <span className="text-sm font-bold text-gray-200">2 Year Pro Warranty</span>
                </div>
                <div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
          ...
          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12">
              <span className="text-blue-400 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">Masterpiece Specs</span>
              <h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2>
              <div className="w-32 h-1.5 bg-gradient-to-r from-blue-500 via-cyan-400 to-blue-500 rounded-full"></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {specs.map((spec, idx) => (
                <motion.div key={idx} whileHover={{ y: -10, backgroundColor: 'rgba(255,255,255,0.08)', borderColor: 'rgba(255,255,255,0.2)' }} className="p-10 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-2xl flex flex-col gap-6 transition-all duration-500 group shadow-2xl">
                  ...
                  <div className="text-left"><h4 className="text-blue-400/80 text-[10px] font-black uppercase tracking-[0.2em] mb-2">{spec.label}</h4><p className="text-white text-2xl font-black tracking-tight group-hover:text-orange-400 transition-colors duration-500">{spec.value}</p></div>
                </motion.div>
              ))}
            </div>

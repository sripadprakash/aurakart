...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiSettings, FiWind, FiCloudLightning, FiSmartphone, FiCpu } from 'react-icons/fi';
...
const AuraSmartBed = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Somnus Smart Bed added to cart!');
  };
  ...
  function handleCompletePurchase() {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (5999.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success("Purchase Successful! Ultimate Sleep is on its way.", {
        icon: '🌙',
        duration: 5000,
  ...
  const specs = [
    { label: "Climate", value: "Dual-Zone Active Thermal Control", icon: <FiWind className="text-cyan-400" /> },
    { label: "Base", value: "Zero-Gravity Zero-G Adjust", icon: <FiActivity className="text-purple-400" /> },
    { label: "Tracking", value: "Integrated Bio-Metric Sensors", icon: <FiCpu className="text-blue-400" /> },
    { label: "App", value: "Aura Home iOS/Android Control", icon: <FiSmartphone className="text-emerald-400" /> },
    { label: "Massage", value: "Multi-Point Sonic Vibrancy", icon: <FiZap className="text-orange-400" /> },
    { label: "Warranty", value: "25 Year Core Component Care", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Somnus Ultimate Edition</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Bed" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Smart Bed" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-amber-500/20 text-amber-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-amber-500/30">Science of Sleep</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Somnus <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400">Integrated Smart Bed</span></h1>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="text-gray-400 font-medium">(5.0k Reviews)</span>
                </div>
              </div>
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The last bed you'll ever buy. The Aura Somnus features active dual-zone thermal control, AI-powered sleep tracking, and a zero-gravity adjustable base that completely redefines the meaning of rest.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-amber-500/20 text-amber-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Elite Grade</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-amber-400 text-2xl" /><span className="text-sm font-bold text-gray-200">25 Year Core Component Cover</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">White-Glove Premium Install</span></div></div>
            </motion.div>
          </div>

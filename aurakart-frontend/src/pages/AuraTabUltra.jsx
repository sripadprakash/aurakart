...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiSmartphone, FiCpu, FiMonitor } from 'react-icons/fi';
...
const AuraTabUltra = () => {
  ...
  const product = {
    id: 'elec-tablet-1',
    name: "AuraTab Ultra 13\" (OLED, 120Hz)",
    price: 749.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('AuraTab added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      addOrder({
        total: (749.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Your digital canvas is ready.', {
        icon: '📱',
        duration: 5000,
  ...
  const specs = [
    { label: "Display", value: "13\" Dynamic OLED 120Hz", icon: <FiMonitor className="text-orange-400" /> },
    { label: "Processor", value: "Snapdragon 8 Gen 3", icon: <FiCpu className="text-blue-400" /> },
    { label: "Design", value: "Ultra-Slim 5.5mm Body", icon: <FiSmartphone className="text-purple-400" /> },
    { label: "Battery", value: "10,000 mAh All-Day", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Audio", value: "Quad Studio Speakers", icon: <FiZap className="text-cyan-400" /> },
    { label: "Warranty", value: "2 Year Global Warranty", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Flagship Tablet Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Tablet" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Tablet" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-video" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Premium Productivity Tablet</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">AuraTab <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-400">Ultra 13" (OLED)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Experience ultimate versatility. AuraTab Ultra 13" features a breathtaking OLED display, powerful Snapdragon performance, and a sleek ultra-slim design for work and play.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-orange-500/20 text-orange-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Best Value</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">2 Year Pro Warranty</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Global Priority Shipping</span></div></div>
            </motion.div>
          </div>

...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiSettings, FiSun, FiWind, FiBattery } from 'react-icons/fi';
...
const AuraFloatingLamp = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Floating Lamp added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Illumination is on its way.', {
        icon: '💡',
        duration: 5000,
  ...
  const specs = [
    { label: "Tech", value: "Electromagnetic Levitation", icon: <FiZap className="text-cyan-400" /> },
    { label: "Control", value: "Seamless Touch Dimming", icon: <FiSun className="text-amber-400" /> },
    { label: "Light", value: "16M Color Aura Spectrum", icon: <FiActivity className="text-purple-400" /> },
    { label: "Power", value: "Wireless Induction Drive", icon: <FiBattery className="text-emerald-400" /> },
    { label: "Material", value: "Aero-Grade Brushed Steel", icon: <FiSettings className="text-blue-400" /> },
    { label: "Warranty", value: "3 Year Advanced Exchange", icon: <FiShield className="text-red-400" /> },
  ];
  ...
  return (
    <div className="min-h-screen text-white font-sans flex flex-col relative overflow-hidden">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Gravity Illumination Series</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Lamp" />
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-center"><span className="text-lg font-bold">Total</span><span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400">${((product.price) + (user?.orders?.length >= 3 ? 1 : 0)).toFixed(2)}</span></div>
                </div>
      ...
      <div className="fixed inset-0 z-0 bg-[#020617] overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[45rem] h-[45rem] bg-amber-600/10 rounded-full blur-[150px]"></div>
      </div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-cyan-500 to-amber-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src={product.image} alt="Floating Lamp" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-cyan-500/20 text-cyan-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-cyan-500/30">Future Decor</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Gravity <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-amber-400">Floating Tech Lamp</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Defy the laws of physics. The Aura Gravity lamp uses precision electromagnetic induction to levitate and power its LED bulb in mid-air, providing a mesmerizing glow and a centerpiece that sparks conversation.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Innovation Choice</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-emerald-400 text-2xl" /><span className="text-sm font-bold text-gray-200">3 Year Advanced Exchange</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Fragile Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-cyan-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-cyan-500/10 rounded-full border border-cyan-500/20">Artisan Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-cyan-500 via-amber-400 to-cyan-500 rounded-full shadow-[0_0_20px_rgba(6,182,212,0.4)]"></div></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {specs.map((spec, idx) => (
                <motion.div key={idx} whileHover={{ y: -10, backgroundColor: 'rgba(255,255,255,0.08)', borderColor: 'rgba(255,255,255,0.2)' }} className="p-10 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-2xl flex flex-col gap-6 transition-all duration-500 group shadow-2xl">
                  ...
                  <div className="text-left"><h4 className="text-cyan-400/80 text-[10px] font-black uppercase tracking-[0.2em] mb-2">{spec.label}</h4><p className="text-white text-2xl font-black tracking-tight group-hover:text-cyan-400 transition-colors duration-500">{spec.value}</p></div>
                </motion.div>
              ))}
            </div>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[140%] h-[140%] bg-cyan-600/5 blur-[180px] -z-10 pointer-events-none rounded-full"></div>
          </section>
        </main>
        <Footer />
      </div>
...
export default AuraFloatingLamp;

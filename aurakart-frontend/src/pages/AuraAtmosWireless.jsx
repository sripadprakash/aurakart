...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiVolume2, FiBluetooth, FiMic } from 'react-icons/fi';
...
const AuraAtmosWireless = () => {
  ...
  const product = {
    id: 'gaming-headset-1',
    name: "Aura Atmos 7.1 Wireless (Spatial Audio)",
    price: 299.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Atmos Wireless added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Immerse yourself now.', {
        icon: '🎧',
        duration: 5000,
  ...
  const specs = [
    { label: "Audio", value: "Aura Atmos 7.1 Spatial", icon: <FiVolume2 className="text-orange-400" /> },
    { label: "Drivers", value: "50mm Planar Magnetic", icon: <FiZap className="text-blue-400" /> },
    { label: "Mic", value: "Broadcast-Grade Boom", icon: <FiMic className="text-purple-400" /> },
    { label: "Battery", value: "60-Hour Hyper-Life", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Wireless", value: "2.4GHz Zero-Loss Tech", icon: <FiBluetooth className="text-cyan-400" /> },
    { label: "Warranty", value: "2 Year Global Support", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Spatial Audio Gear Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Headset" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-indigo-500 to-rose-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src={product.image} alt="Headset" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-indigo-500/20 text-indigo-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-indigo-500/30">High-Fidelity Spatial Audio</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Atmos <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-rose-400">7.1 Wireless</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Hear the victory. The Aura Atmos 7.1 Wireless delivers pinpoint spatial accuracy with planar magnetic drivers and 60 hours of continuous hyper-life battery for the longest gaming sessions.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-indigo-500/20 text-indigo-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Spatial Grade</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-indigo-400 text-2xl" /><span className="text-sm font-bold text-gray-200">2 Year Audio Warranty</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Priority Delivery</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-indigo-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-indigo-500/10 rounded-full border border-indigo-500/20">Acoustic Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-indigo-500 via-rose-400 to-indigo-500 rounded-full shadow-[0_0_20px_rgba(99,102,241,0.4)]"></div></div>
...
export default AuraAtmosWireless;

...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiCpu, FiSmartphone, FiBattery, FiCamera, FiWifi, FiActivity, FiHardDrive, FiMapPin, FiEdit3 } from 'react-icons/fi';
...
const AuraUltra24 = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Aura Ultra 24 added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (1199) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! The Ultra Flagship is on its way.', {
        icon: '🚀',
        duration: 5000,
  ...
  const specs = [
    { label: "Processor", value: "Aura Z1 Extreme Chip", icon: <FiCpu className="text-orange-400" /> },
    { label: "Memory", value: "16GB RAM, 512GB", icon: <FiHardDrive className="text-blue-400" /> },
    { label: "Camera", value: "200MP Ultra Vision", icon: <FiCamera className="text-purple-400" /> },
    { label: "Battery", value: "5500 mAh", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Network", value: "Next-Gen 5G & Wi-Fi 7", icon: <FiWifi className="text-cyan-400" /> },
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Ultra Flagship Edition (16GB/512GB)</p></div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Ultra 24" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-blue-500/30">16GB RAM, 512GB Edition</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Ultra 24 (16GB RAM, 512GB)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The pinnacle of performance. The Aura Ultra 24 (16GB RAM, 512GB) redefines what a smartphone can be with a built-in stylus, 200MP sensor, and the groundbreaking Z1 Extreme chip.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Premium Choice</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">3 Year AuraCare+</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Priority Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-blue-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">Masterpiece Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-blue-500 via-cyan-400 to-blue-500 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.4)]"></div></div>

...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiTarget, FiCommand, FiCpu } from 'react-icons/fi';
...
const AuraGlideZeroMouse = () => {
  ...
  const product = {
    id: 'gaming-mouse-1',
    name: "Aura Glide-Zero Mouse (32K DPI, 45g)",
    price: 149.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Glide-Zero Mouse added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Precision tool is on its way.', {
        icon: '🖱️',
        duration: 5000,
  ...
  const specs = [
    { label: "Sensor", value: "Aura-Vision 32K Elite", icon: <FiTarget className="text-orange-400" /> },
    { label: "Weight", value: "45g Ultra-Lightweight", icon: <FiActivity className="text-blue-400" /> },
    { label: "Switches", value: "100M Click Optical", icon: <FiZap className="text-purple-400" /> },
    { label: "Polling", value: "8000Hz Wireless Tech", icon: <FiCpu className="text-green-400" /> },
    { label: "Buttons", value: "6 Programmable Macros", icon: <FiCommand className="text-cyan-400" /> },
    { label: "Warranty", value: "2 Year Pro Coverage", icon: <FiShield className="text-red-400" /> },
  ];
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Mouse" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Zero Mouse" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-indigo-500/20 text-indigo-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-indigo-500/30">Precision Engineering</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Glide <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Zero Mouse</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Break the limits of speed. At only 45 grams and featuring a 32,000 DPI sensor, the Aura Glide-Zero is engineered for pixel-perfect accuracy and instantaneous response in high-stakes competitive gaming.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-indigo-500/20 text-indigo-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Pro Edition</span></div>
          ...
          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-indigo-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-indigo-500/10 rounded-full border border-indigo-500/20">Performance Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-indigo-500 via-purple-400 to-indigo-500 rounded-full shadow-[0_0_20px_rgba(99,102,241,0.4)]"></div></div>
...
export default AuraGlideZeroMouse;

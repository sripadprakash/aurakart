...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiActivity, FiMapPin, FiCamera, FiVideo, FiAperture, FiTarget } from 'react-icons/fi';
...
const AuraSnapX1 = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('AuraSnap X1 added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      addOrder({
        total: (1099.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Capture your masterpieces now.', {
        icon: '📷',
        duration: 5000,
  ...
  const specs = [
    { label: "Sensor", value: "24.2MP Full-Frame CMOS", icon: <FiCamera className="text-orange-400" /> },
    { label: "Video", value: "4K/60p 10-bit Internal", icon: <FiVideo className="text-blue-400" /> },
    { label: "Autofocus", value: "AI Subject Tracking", icon: <FiTarget className="text-purple-400" /> },
    { label: "Speed", value: "10 fps Continuous Shooting", icon: <FiZap className="text-green-400" /> },
    { label: "ISO Range", value: "100 - 51,200 Native", icon: <FiAperture className="text-cyan-400" /> },
    { label: "Display", value: "3.0\" Vari-angle LCD", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Imaging Collection Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Camera" />
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-orange-400 font-bold">-$200 OFF</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-sm"><span className="text-gray-400 font-medium">Original Price</span><span className="font-bold text-gray-500 line-through">$1,299.99</span></div>
                  <div className="flex justify-between text-sm text-orange-400 font-bold"><span className="font-medium">Hero Discount</span><span>-$200.00</span></div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Camera" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Pro Performance</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">AuraSnap <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-400">X1 Mirrorless</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Master your craft. AuraSnap X1 features a high-resolution 24.2MP sensor and advanced AI subject tracking for breathtaking clarity in every shot.</p>
              
              <div className="flex items-center gap-6 my-4">
                <span className="text-5xl font-black text-white">${product.price}</span>
                <div className="flex flex-col items-start">
                  <span className="text-xl text-gray-500 line-through font-bold">${product.originalPrice}</span>
                  <span className="text-orange-400 text-sm font-black uppercase tracking-wider text-left">Save $200.00 Today</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">2 Year Pro Support</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Global Priority Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-orange-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-orange-500/10 rounded-full border border-orange-500/20">X-Series Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-orange-500 via-rose-400 to-orange-500 rounded-full shadow-[0_0_20px_rgba(249,115,22,0.4)]"></div></div>

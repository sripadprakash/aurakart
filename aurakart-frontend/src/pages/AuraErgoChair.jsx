...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiSettings, FiGrid, FiLayers, FiAnchor } from 'react-icons/fi';
...
const AuraErgoChair = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Zenith Ergo Chair added to cart!');
  };
  ...
  function handleCompletePurchase() {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (899.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success("Purchase Successful! Your Posture's Savior is coming.", {
        icon: '💺',
        duration: 5000,
  ...
  const specs = [
    { label: "Lumbar", value: "Dynamic 4D Adaptive Support", icon: <FiAnchor className="text-amber-400" /> },
    { label: "Material", value: "Aura-Breathe™ Tech Mesh", icon: <FiLayers className="text-orange-400" /> },
    { label: "Adjust", value: "15-Point Multi-Lock System", icon: <FiSettings className="text-purple-400" /> },
    { label: "Base", value: "Polished Industrial Aluminum", icon: <FiShield className="text-yellow-400" /> },
    { label: "Motion", value: "Silent-Glide 360° Precision", icon: <FiActivity className="text-cyan-400" /> },
    { label: "Warranty", value: "10 Year Performance Cover", icon: <FiCheckCircle className="text-emerald-400" /> },
  ];
  ...
  return (
    <div className="min-h-screen text-white font-sans flex flex-col relative overflow-hidden">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Zenith Performance Seating</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Chair" />
                  <div className="flex-1">
                    <h4 className="font-bold text-sm">{product.name}</h4>
                    <div className="flex items-center gap-2 mt-1">
                      <FiMapPin className="text-amber-400" size={12} />
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-sm"><span className="text-gray-400 font-medium">Subtotal</span><span className="font-bold">${product.price}</span></div>
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-center"><span className="text-lg font-bold">Total</span><span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400">${((product.price) + (user?.orders?.length >= 3 ? 1 : 0)).toFixed(2)}</span></div>
                </div>
                <button onClick={handleCompletePurchase} disabled={isProcessing} className={`w-full py-5 rounded-2xl font-black text-lg transition-all duration-300 flex items-center justify-center gap-3 ${isProcessing ? 'bg-white/10 text-white/30' : 'bg-white text-black hover:bg-amber-500 hover:text-white shadow-xl shadow-amber-500/20'}`}>
                  {isProcessing ? (<><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full" /><span>PREPARING...</span></>) : (<><FiZap className="text-xl" />{location === 'Select Location' ? 'SELECT ADDRESS' : 'COMPLETE PURCHASE'}</>)}
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <div className="fixed inset-0 z-0 bg-[#020617] overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40rem] h-[40rem] bg-amber-600/10 rounded-full blur-[120px]"></div>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          <div className="flex justify-between items-center mb-8">
            <Link to="/category/furniture" className="inline-flex items-center gap-2 text-gray-400 hover:text-white transition-colors group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              <div className="absolute -inset-4 bg-gradient-to-r from-amber-500 to-orange-600 rounded-3xl blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              ...
              <img src={product.image} alt="Zenith Chair" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-amber-500/20 text-amber-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-amber-500/30">Ergonomic Excellence</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Zenith <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400">Integrated Ergo Elite</span></h1>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="text-gray-400 font-medium">(4.8k Reviews)</span>
                </div>
              </div>
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The science of sitting. The Aura Zenith is engineered with breathable tech-mesh and dynamic 4D lumbar support that adjusts to your every move, ensuring total comfort through your most intense work sessions.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-amber-500/20 text-amber-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Pro Choice</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-amber-400 text-2xl" /><span className="text-sm font-bold text-gray-200">10 Year Performance Cover</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Priority Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-amber-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-amber-500/10 rounded-full border border-amber-500/20">Artisan Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-amber-500 via-orange-400 to-amber-500 rounded-full shadow-[0_0_20px_rgba(245,158,11,0.4)]"></div></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {specs.map((spec, idx) => (
                <motion.div key={idx} whileHover={{ y: -10, backgroundColor: 'rgba(255,255,255,0.08)', borderColor: 'rgba(255,255,255,0.2)' }} className="p-10 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-2xl flex flex-col gap-6 transition-all duration-500 group shadow-2xl">
                  ...
                  <div className="text-left"><h4 className="text-amber-400/80 text-[10px] font-black uppercase tracking-[0.2em] mb-2">{spec.label}</h4><p className="text-white text-2xl font-black tracking-tight group-hover:text-amber-400 transition-colors duration-500">{spec.value}</p></div>
                </motion.div>
              ))}
            </div>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[140%] h-[140%] bg-amber-600/5 blur-[180px] -z-10 pointer-events-none rounded-full"></div>
          </section>
        </main>
        <Footer />
      </div>

...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiNavigation, FiVideo, FiWind } from 'react-icons/fi';
...
const AuraSkyGuardian = () => {
  ...
  const product = {
    id: 'elec-drone-1',
    name: "AuraSky Guardian Drone (4K/60fps, 45min)",
    price: 899.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('AuraSky Drone added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Take to the skies.', {
        icon: '🛸',
        duration: 5000,
  ...
  const specs = [
    { label: "Camera", value: "4K/60fps HDR Cinema", icon: <FiVideo className="text-orange-400" /> },
    { label: "Flight Time", value: "45 Minutes Ultra-Long", icon: <FiActivity className="text-blue-400" /> },
    { label: "Range", value: "15km OcuSync 4.0", icon: <FiNavigation className="text-purple-400" /> },
    { label: "Battery", value: "Intelligent Flight Pack", icon: <FaBatteryFull className="text-green-400" /> },
    { label: "Stability", value: "Level 7 Wind Resistance", icon: <FiWind className="text-cyan-400" /> },
    { label: "Warranty", value: "2 Year FlySafe+", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">AuraSky Aerial Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Drone" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Drone" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-video" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Next-Gen Aerial Guardian</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">AuraSky <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-400">Guardian Drone (4K)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Master the skies. AuraSky Guardian Drone features a 4K cinema camera, 45-minute flight time, and advanced obstacle avoidance for the ultimate aerial experience.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-orange-500/20 text-orange-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Elite Tech</span></div>
...
export default AuraSkyGuardian;

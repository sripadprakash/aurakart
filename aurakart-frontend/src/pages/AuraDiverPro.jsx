...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiAnchor, FiCompass, FiLayers } from 'react-icons/fi';
...
const AuraDiverPro = () => {
  ...
  const product = {
    id: 'watch-sport-1',
    name: "Aura Diver Pro 300M (Ceramic Bezel)",
    price: 1299.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Diver Pro added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      addOrder({
        total: (1299.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Undersea Tool Secured.', {
        icon: '🌊',
        duration: 5000,
  ...
  const specs = [
    { label: "Depth", value: "300M (1000ft) ISO Rated", icon: <FiAnchor className="text-orange-400" /> },
    { label: "Bezel", value: "Ceramic Scratch-Proof", icon: <FiLayers className="text-blue-400" /> },
    { label: "Lume", value: "Super-Luminova BGW9", icon: <FiZap className="text-purple-400" /> },
    { label: "Valve", value: "Helium Escape System", icon: <FiActivity className="text-green-400" /> },
    { label: "Movement", value: "Aura Swiss-Tech Auto", icon: <FiCompass className="text-cyan-400" /> },
    { label: "Warranty", value: "Lifetime Seal Warranty", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Professional Diver Gear</p></div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Diver Pro" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-orange-500/20 text-orange-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-orange-500/30">Professional Diver</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Diver <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-400">Pro 300M (Ceramic)</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Built for the abyss. The Aura Diver Pro combines extreme water resistance with professional-grade ceramic materials, ensuring reliability in the deepest environments known to man.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-orange-500/20 text-orange-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Pro Choice</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Lifetime Seal Warranty</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Express Secure Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-orange-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-orange-500/10 rounded-full border border-orange-500/20">Survival Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-orange-500 via-amber-400 to-orange-500 rounded-full shadow-[0_0_20px_rgba(249,115,22,0.4)]"></div></div>

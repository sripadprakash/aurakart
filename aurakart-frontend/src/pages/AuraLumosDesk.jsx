...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiCpu, FiSmartphone, FiBattery, FiHardDrive, FiSettings, FiMonitor } from 'react-icons/fi';
...
const AuraLumosDesk = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Lumos Smart Desk added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (1499.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Your Smart Workspace is coming.', {
        icon: '🖥️',
        duration: 5000,
  ...
  const specs = [
    { label: "Control", value: "Integrated OLED Touch Strip", icon: <FiSmartphone className="text-blue-400" /> },
    { label: "Charging", value: "Triple 15W Wireless Pads", icon: <FiZap className="text-cyan-400" /> },
    { label: "Lift", value: "Dual-Motor Silent Height Adjust", icon: <FiSettings className="text-purple-400" /> },
    { label: "Connectivity", value: "Built-in 10-Port USB-C Hub", icon: <FiHardDrive className="text-blue-400" /> },
    { label: "Surface", value: "Anti-Glare Tempered Glass", icon: <FiMonitor className="text-emerald-400" /> },
    { label: "Warranty", value: "5 Year Electronics Coverage", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Lumos Workspace Ecosystem</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Desk" />
                ...
                <div className="space-y-3">
                  ...
                  <div className="pt-3 border-t border-white/5 flex justify-between items-center"><span className="text-lg font-bold">Total</span><span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">${((product.price) + (user?.orders?.length >= 3 ? 1 : 0)).toFixed(2)}</span></div>
                </div>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Lumos Desk" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-blue-500/30">Next-Gen Workspace</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Lumos <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Integrated Smart Desk</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The workstation of the future. The Aura Lumos features an integrated OLED touch strip for height and lighting control, built-in wireless charging across the entire surface, and a unified 10-port hub for the ultimate clean setup.</p>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">5 Year Tech Warranty</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-cyan-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Insured Premium Shipping</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-blue-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">Artisan Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Technical Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-blue-500 via-cyan-400 to-blue-500 rounded-full shadow-[0_0_20px_rgba(59,130,246,0.4)]"></div></div>

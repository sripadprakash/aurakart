...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiMonitor, FiEye, FiTv } from 'react-icons/fi';
...
const AuraVisionCurved = () => {
  ...
  const product = {
    id: 'elec-monitor-1',
    name: "AuraVision 49\" Curved (240Hz, 1ms, OLED)",
    price: 1299.99,
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('AuraVision Monitor added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Purchase Successful! Your setup is now complete.', {
        icon: '🖥️',
        duration: 5000,
  ...
  const specs = [
    { label: "Panel", value: "49\" Super Ultra-Wide OLED", icon: <FiMonitor className="text-orange-400" /> },
    { label: "Refresh", value: "240Hz Lightning Fast", icon: <FiZap className="text-blue-400" /> },
    { label: "Response", value: "0.03ms (GtG) Elite", icon: <FiActivity className="text-purple-400" /> },
    { label: "Curve", value: "1000R Immersive Arc", icon: <FiEye className="text-green-400" /> },
    { label: "HDR", value: "DisplayHDR True Black 400", icon: <FiTv className="text-cyan-400" /> },
    { label: "Warranty", value: "3 Year Panel Coverage", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Display Solution Order</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Monitor" />
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Monitor" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-video" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-blue-500/20 text-blue-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-blue-500/30">Immersive Visual Hub</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">AuraVision <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">49" Curved OLED</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">The ultimate display for gaming and productivity. AuraVision 49" Curved features a 240Hz refresh rate, 1ms response time, and a breathtaking Dual QHD OLED panel.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Immersive Grade</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-orange-400 text-2xl" /><span className="text-sm font-bold text-gray-200">3 Year Pixel Warranty</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Insured Heavy Delivery</span></div></div>
            </motion.div>
          </div>

          <section className="mt-24 relative">
            <div className="flex flex-col items-center text-center mb-12"><span className="text-orange-500 text-sm font-black tracking-[0.4em] uppercase mb-4 px-4 py-1 bg-orange-500/10 rounded-full border border-orange-500/20">Display Tech Specs</span><h2 className="text-5xl md:text-6xl font-black text-white mb-6 tracking-tight">Visual Excellence</h2><div className="w-32 h-1.5 bg-gradient-to-r from-orange-500 via-rose-400 to-orange-500 rounded-full shadow-[0_0_20px_rgba(249,115,22,0.4)]"></div></div>
...
export default AuraVisionCurved;

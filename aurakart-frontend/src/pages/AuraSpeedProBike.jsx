...
import { FiArrowLeft, FiStar, FiShield, FiTruck, FiHeart, FiZap, FiX, FiCheckCircle, FiActivity, FiMapPin, FiSettings, FiTarget, FiFeather, FiLayers, FiWind } from 'react-icons/fi';
...
const AuraSpeedProBike = () => {
  ...
  const handleAddToCart = () => {
    ...
    setIsAdded(true);
    toast.success('Speed-Pro Carbon Bike added to cart!');
  };
  ...
  const handleCompletePurchase = () => {
    ...
    setTimeout(() => {
      ...
      // RECORD ORDER
      addOrder({
        total: (3499.99) + (user?.orders?.length >= 3 ? 1 : 0),
        items: 1,
        productName: product.name
      });

      toast.success('Purchase Successful! Your Elite Ride is being prepared.', {
        icon: '🚲',
        duration: 5000,
  ...
  const specs = [
    { label: "Frame", value: "Military-Grade T1000 Carbon", icon: <FiLayers className="text-emerald-400" /> },
    { label: "Groupset", value: "Shimano Dura-Ace Di2 22-Speed", icon: <FiSettings className="text-blue-400" /> },
    { label: "Aero", value: "Wind-Tunnel Optimized Design", icon: <FiWind className="text-purple-400" /> },
    { label: "Weight", value: "Ultralight 6.8kg Total Mass", icon: <FiFeather className="text-green-400" /> },
    { label: "Brakes", value: "Hydraulic Stealth-Disc System", icon: <FiTarget className="text-cyan-400" /> },
    { label: "Warranty", value: "Lifetime Frame Protection", icon: <FiShield className="text-red-400" /> },
  ];
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-[#0f172a] border border-white/10 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 flex justify-between items-center">
                <div><h2 className="text-2xl font-black">Secure Checkout</h2><p className="text-gray-400 text-sm font-medium">Elite Carbon Performance</p></div>
              ...
              <div className="p-8 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                  <img src={product.image} className="w-16 h-16 rounded-xl object-cover" alt="Bike" />
                  <div className="flex-1">
                    <h4 className="font-bold text-sm">{product.name}</h4>
                    <div className="flex items-center gap-2 mt-1">
                      <FiMapPin className="text-emerald-400" size={12} />
                ...
                <button onClick={handleCompletePurchase} disabled={isProcessing} className={`w-full py-5 rounded-2xl font-black text-lg transition-all duration-300 flex items-center justify-center gap-3 ${isProcessing ? 'bg-white/10 text-white/30' : 'bg-white text-black hover:bg-emerald-500 hover:text-white shadow-xl shadow-emerald-500/20'}`}>
        ...
        <main className="flex-grow pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative group">
              ...
              <img src={product.image} alt="Speed-Pro Bike" className="relative rounded-2xl shadow-2xl border border-white/10 w-full object-cover aspect-square" />
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex flex-col gap-6">
              <div>
                <span className="bg-emerald-500/20 text-emerald-400 px-4 py-1.5 rounded-md text-xs font-bold tracking-widest uppercase mb-4 inline-block border border-emerald-500/30">Elite Performance</span>
                <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 text-white text-left leading-tight">Aura Speed-Pro <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">Carbon Fiber Elite</span></h1>
              ...
              <p className="text-xl text-gray-300 leading-relaxed font-medium text-left">Master the road. Engineered with military-grade T1000 carbon fiber and the legendary Shimano Dura-Ace Di2 groupset, the Speed-Pro is the pinnacle of aerodynamic efficiency and power transfer.</p>
              <div className="flex items-center gap-6 my-4"><span className="text-5xl font-black text-white">${product.price}</span><span className="bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase">Elite Choice</span></div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8"><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiShield className="text-emerald-400 text-2xl" /><span className="text-sm font-bold text-gray-200">Lifetime Frame Support</span></div><div className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md"><FiTruck className="text-blue-400 text-2xl" /><span className="text-sm font-bold text-gray-200">White-Glove Priority Delivery</span></div></div>
            </motion.div>
          </div>
...
export default AuraSpeedProBike;
